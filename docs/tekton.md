...
2. Create two files named `opaque.env` and `file.secrets` using the snippets below.

    **opaque.secrets**
    Creates a secret for every secret defined. the `key` refers to the secret name, and the `value` is the secret contents.

   ```bash
   cat <<EOF >./overlays/secrets/opaque.secrets
   # Add files in the format key=value... 
   # Each line item will be created as it's own secret...
    ...
    **file.secrets**  
    Creates a secret for every secret defined. the `key` refers to the secret name, and the `value` is the path which refers to the secret location.

   ```bash
   cat <<EOF >./overlays/secrets/file.secrets
   # Add files in the format key=value...
   # Each line item will be created as it's own secret...
...
+yq_darwin_arm64 was installed...
secret/docker-config-path configured
secret/tkn-github-secret configured
secret/tkn-github-token configured
secret/tkn-sonar-token created
